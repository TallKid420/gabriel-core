...
@pytest.fixture
def WRITE_HEADERS(make_auth_headers) -> dict[str, str]:
        return make_auth_headers(
                org="acme",
                identifier="alice",
                capabilities=("read_resource", "write_resource"),
                correlation_id="11111111-1111-1111-1111-111111111111",
        )


@pytest.fixture
def READ_HEADERS(make_auth_headers) -> dict[str, str]:
        return make_auth_headers(
                org="acme",
                identifier="bob",
                capabilities=("read_resource",),
                correlation_id="22222222-2222-2222-2222-222222222222",
        )


def test_upload_document_creates_resource_and_event(client, WRITE_HEADERS, READ_HEADERS):
...
def test_upload_document_denied_without_write_capability(client, READ_HEADERS):
        files = {"file": ("notes.txt", b"hello", "text/plain")}
        response = client.post("/documents", headers=READ_HEADERS, files=files)
        assert response.status_code == 403


def test_uploaded_document_is_retrievable(client, WRITE_HEADERS, READ_HEADERS):
